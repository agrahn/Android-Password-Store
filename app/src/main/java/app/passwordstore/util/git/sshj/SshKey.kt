...
object SshKey {
  ...
  fun generateKeystoreWrappedEd25519Key(requireAuthentication: Boolean) {
    ...
    context.gitSecrets.edit {
      putString(
        KEYSTORE_ALIAS,
        "${requireAuthentication}:" + seedEncrypted,
      )
    }
  ...
  private fun provideKeystoreWrappedEd25519Key(client: SSHClient): KeyProvider =
    runCatching {
        ...
        val mustAuthenticateAndSeedEncrypted =
          context.gitSecrets.getString(KEYSTORE_ALIAS, "false:")?.split(":", limit = 2)

        val seedDecrypted =
          AESEncryption.decryptToByteArray(
            mustAuthenticateAndSeedEncrypted?.getOrNull(1)?.toCharArray(),
            if (mustAuthenticateAndSeedEncrypted?.getOrNull(0) == "true")
        ...
        val privateKey =
          EdDSAPrivateKey(
            EdDSAPrivateKeySpec(seedDecrypted, EdDSANamedCurveTable.ED_25519_CURVE_SPEC)
          )
