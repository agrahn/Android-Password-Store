...
constructor(filesDir: String, private val dispatcher: CoroutineDispatcher) :
  ...
  override fun addKey(key: PGPKey, replace: Boolean): Result<PGPKey, Throwable> = runCatching {
    ...
    if (keyFile.exists()) {
      val existingKey = PGPKey(keyFile.readBytes())
      when {
        ...
        !hasSecretKey(existingKey) && !hasSecretKey(key) -> {
          val incomingCertificate =
            tryParseCertificateOrKey(key) as OpenPGPCertificate ?: throw InvalidKeyException
          val existingCertificate =
            tryParseCertificateOrKey(existingKey) as OpenPGPCertificate ?: throw InvalidKeyException
          val updatedCertificate = OpenPGPCertificate.join(existingCertificate, incomingCertificate)
